...
async def run_agent_with_retry(agent, prompt, ctx):
...
# Tool to decide the next action
...
async def decide_next_action(
    ...
    Returns the next action to take along with reasoning.
    """
    prompt = f"""
    Based on the current state of the software development process, decide the next action to take.
    ...
    Choose the action that would be most valuable at this stage of development.
    Consider prerequisites for each action and the current development context.
...
# Tool to generate final summary
@coordinator_agent.tool
async def generate_summary(
    ...
    try:
        ...
        summary = json.loads(result.data)
        return {
        ...
        return {
            "key_features": ["Feature extraction failed"],
...
async def develop_software(user_request: str, language: str = "Python", max_iterations: int = 3) -> Dict[str, Any]:
    """
    Main function that orchestrates the development process using the coordinator agent's tools.
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        # Prepare current state for coordinator agent
        current_state = {
        ...
        if action == "analyze_requirements":
            ...
            dev_context.review_issues = await review_code(
                run_ctx, dev_context.code, dev_context.requirements
            )
            print(f"✅ Code review completed - found {len(dev_context.review_issues)} issues")
            
        elif action == "create_tests":
            dev_context.test_cases = await create_tests(
                run_ctx, dev_context.requirements, dev_context.code
            )
            print("✅ Test creation completed")
            
        elif action == "check_security":
            dev_context.security_issues = await check_security(
                run_ctx, dev_context.code, dev_context.requirements
            )
            print(f"✅ Security check completed - found {len(dev_context.security_issues)} issues")
            
        elif action == "optimize_performance":
            dev_context.performance_issues = await optimize_performance(
                run_ctx, dev_context.code, dev_context.requirements
            )
            print(f"✅ Performance optimization completed - found {len(dev_context.performance_issues)} issues")
            
        elif action == "make_improvements":
            dev_context.code = await make_improvements(
                run_ctx, 
                dev_context.code, 
    ...
    summary = await generate_summary(run_ctx, user_request, language, dev_context)
